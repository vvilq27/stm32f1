...
void ST7735_Init(void) {
	...
	PORT.GPIO_Pin = ST7735_CS_PIN; //PIN 7 as CS
	GPIO_Init(ST7735_CS_PORT,&PORT);
	PORT.GPIO_Pin = ST7735_RST_PIN;	//PIN 6 as RESET
	...
	/*		setting displays registers goes like:
	 * first we refer register we want to write(st7735_cmd)
	 * then goes for example 3 parameters, one after another to that regiseter(st7735_write)
	 * then again cmd to change register and so on
	 */
